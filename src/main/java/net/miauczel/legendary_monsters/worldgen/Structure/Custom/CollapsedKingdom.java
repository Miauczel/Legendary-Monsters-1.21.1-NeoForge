...
public class CollapsedKingdom extends Structure {
    ...
    public static final MapCodec<CollapsedKingdom> CODEC = RecordCodecBuilder
            .<CollapsedKingdom>mapCodec(instance -> instance.group(
                    Structure.<CollapsedKingdom>settingsCodec(instance),

                    StructureTemplatePool.CODEC
                            .fieldOf("start_pool")
                            .forGetter((CollapsedKingdom structure) -> structure.startPool),

                    ResourceLocation.CODEC
                            .optionalFieldOf("start_jigsaw_name")
                            .forGetter((CollapsedKingdom structure) -> structure.startJigsawName),

                    Codec.intRange(MIN_DEPTH, MAX_DEPTH)
                            .fieldOf("size")
                            .forGetter((CollapsedKingdom structure) -> structure.maxDepth),

                    HeightProvider.CODEC
                            .fieldOf("start_height")
                            .forGetter((CollapsedKingdom structure) -> structure.startHeight),

                    Codec.BOOL
                            .fieldOf("use_expansion_hack")
                            .forGetter((CollapsedKingdom structure) -> structure.useExpansionHack),

                    Heightmap.Types.CODEC
                            .optionalFieldOf("project_start_to_heightmap")
                            .forGetter((CollapsedKingdom structure) -> structure.projectStartToHeightmap),

                    Codec.intRange(1, MAX_TOTAL_STRUCTURE_RANGE)
                            .fieldOf("max_distance_from_center")
                            .forGetter((CollapsedKingdom structure) -> structure.maxDistanceFromCenter),

                    PoolAliasBinding.CODEC
                            .listOf()
                            .optionalFieldOf("pool_aliases", List.of())
                            .forGetter((CollapsedKingdom structure) -> structure.poolAliases),

                    DimensionPadding.CODEC
                            .optionalFieldOf("dimension_padding", DEFAULT_DIMENSION_PADDING)
                            .forGetter((CollapsedKingdom structure) -> structure.dimensionPadding),

                    LiquidSettings.CODEC
                            .optionalFieldOf("liquid_settings", DEFAULT_LIQUID_SETTINGS)
                            .forGetter((CollapsedKingdom structure) -> structure.liquidSettings)
            ).apply(instance, CollapsedKingdom::new))
            .validate(CollapsedKingdom::verifyRange);
    ...
    private static DataResult<CollapsedKingdom> verifyRange(CollapsedKingdom structure) {
