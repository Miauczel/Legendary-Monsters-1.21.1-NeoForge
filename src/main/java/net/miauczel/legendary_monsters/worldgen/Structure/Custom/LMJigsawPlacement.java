...
public class LMJigsawPlacement {
    ...
    public static Optional<Structure.GenerationStub> addPieces(Structure.GenerationContext p_227239_, Holder<StructureTemplatePool> p_227240_, Optional<ResourceLocation> p_227241_, int p_227242_, BlockPos p_227243_, boolean p_227244_, Optional<Heightmap.Types> p_227245_, int p_227246_) {
        ...
        if (structurepoolelement == EmptyPoolElement.INSTANCE) {
            ...
            PoolElementStructurePiece poolelementstructurepiece = new PoolElementStructurePiece(structuretemplatemanager,
                    structurepoolelement, blockpos1, structurepoolelement.getGroundLevelDelta(), rotation, structurepoolelement.getBoundingBox(structuretemplatemanager, blockpos1, rotation),LiquidSettings.IGNORE_WATERLOGGING);

            BoundingBox initialBox = poolelementstructurepiece.getBoundingBox();

            int initialCenterX = (initialBox.minX() + initialBox.maxX()) / 2;
            int initialCenterZ = (initialBox.minZ() + initialBox.maxZ()) / 2;

// Umieszczamy środek startowego elementu dokładnie w startPos.
            int shiftX = p_227243_.getX() - initialCenterX;
            int shiftZ = p_227243_.getZ() - initialCenterZ;

            poolelementstructurepiece.move(shiftX, 0, shiftZ);

            BoundingBox horizontallyCenteredBox =
                    poolelementstructurepiece.getBoundingBox();

            int centerX =
                    (horizontallyCenteredBox.minX()
                            + horizontallyCenteredBox.maxX()) / 2;

            int centerZ =
                    (horizontallyCenteredBox.minZ()
                            + horizontallyCenteredBox.maxZ()) / 2;

            int targetY;

            if (p_227245_.isPresent()) {
                targetY = p_227243_.getY()
                ...
                targetY = blockpos1.getY();
            }

            int currentGroundY =
                    horizontallyCenteredBox.minY()
                            + poolelementstructurepiece.getGroundLevelDelta();

            poolelementstructurepiece.move(
                    0,
            ...
            int generationY = targetY + vec3i.getY();

            return Optional.of(
                    new Structure.GenerationStub(
                            new BlockPos(centerX, generationY, centerZ),
                            builder -> {
                                ...
                                if (p_227242_ > 0) {
                                    AABB allowedArea = new AABB(
                                            centerX - p_227246_,
                                            generationY - p_227246_,
                                            centerZ - p_227246_,
                                            centerX + p_227246_ + 1,
                                            generationY + p_227246_ + 1,
                                            centerZ + p_227246_ + 1
                                    );

                                    VoxelShape freeSpace = Shapes.join(
                                            Shapes.create(allowedArea),
                                            Shapes.create(AABB.of(finalBox)),
    ...
    static final class Placer {
                                    ...
                                    for (StructureTemplate.StructureBlockInfo structuretemplate$structureblockinfo1 : list1) {
                                        if (JigsawBlock.canAttach(structuretemplate$structureblockinfo, structuretemplate$structureblockinfo1)) {
                                            ...
                                            if (!Shapes.joinIsNotEmpty(mutableobject1.getValue(), Shapes.create(AABB.of(boundingbox3).deflate(0.25D)), BooleanOp.ONLY_SECOND)) {
                                                ...
                                                PoolElementStructurePiece poolelementstructurepiece = new PoolElementStructurePiece(this.structureTemplateManager, structurepoolelement1, blockpos5, k2, rotation1, boundingbox3, LiquidSettings.IGNORE_WATERLOGGING);
                                                int l2;
                                                if (flag) {
                                                    l2 = i + j;
                                                } else if (flag2) {
        ...
        private static ResourceKey<StructureTemplatePool> readPoolName(StructureTemplate.StructureBlockInfo p_256491_) {
            return ResourceKey.create(Registries.TEMPLATE_POOL, ResourceLocation.parse(p_256491_.nbt().getString("pool")));
        }
    }
}