...
public class KickAfterStompStateGoal extends Goal {
...
public int state = 1;
    public KickAfterStompStateGoal(IAnimatedMonster entity, int getattackstate, int attackstate, int attackendstate, int attackfinaltick, int attackseetick) {
        this.entity = entity;
    ...
    public KickAfterStompStateGoal(IAnimatedMonster entity, int getattackstate, int attackstate, int attackendstate, int attackfinaltick, int attackseetick, boolean interruptsAI) {
        this.entity = entity;
        if (interruptsAI) {
            this.setFlags(EnumSet.of(Flag.MOVE, Flag.LOOK, Flag.JUMP));
        }
    ...
    public void start() {
        if (this.getattackstate != this.attackstate) {
            this.entity.setAttackState(getattackstate);
        }

    }

    public void stop() {
        this.entity.setAttackState(state);
    ...
    public void tick() {
        ...
        if (target != null) {
            Vec3 dirToTarget = target.position()
                    .subtract(entity.position())
                    .normalize();
            Vec3 forward    = entity.getLookAngle()
                    .normalize();
            double crossY = forward.cross(dirToTarget).y;
            ...
            if (crossY > 0) {
              ...
              // System.out.println("target is behind");
            }
        }
