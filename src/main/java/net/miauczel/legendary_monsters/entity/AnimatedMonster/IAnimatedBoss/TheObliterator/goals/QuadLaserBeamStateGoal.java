...
public class QuadLaserBeamStateGoal extends Goal {
    ...
    public QuadLaserBeamStateGoal(TheObliteratorEntity entity, int getattackstate, int attackstate, int attackendstate, int attackfinaltick, int attackseetick) {
        this.entity = entity;
    ...
    public QuadLaserBeamStateGoal(TheObliteratorEntity entity, int getattackstate, int attackstate, int attackendstate, int attackfinaltick, int attackseetick, boolean interruptsAI) {
        this.entity = entity;
        if (interruptsAI) {
            this.setFlags(EnumSet.of(Flag.MOVE, Flag.LOOK, Flag.JUMP));
        }
    ...
    public void start() {
        switch (entity.getRandom().nextInt(2)) {
            case 0 -> entity.setIsQuadBeamRight(true);
            case 1 -> entity.setIsQuadBeamRight(false);
            // System.out.println("Returned: " + isRight);
    ...
    public void tick() {
        ...
        if (attackTicks == 10) {
           ...
           entity. playSound(ModSounds.ANNIHILATION_LASER_CHARGE.get(),3,1f);
            if (target !=null) {
                ...
                float vec = 0;
                float offset = entity.getRandom().nextInt()* 100 <50 ? -6: 6;
